...
export async function GET(request) {
  ...
  try {
    ...
    const mentorIdObj = safeObjectId(user?.userId);
    if (!mentorIdObj) {
    ...
    const requests = await collection.find({ mentorId: mentorIdObj }).sort({ createdAt: -1 }).toArray();
    const hydrated = await attachUserDetails(db, requests);

    return NextResponse.json({ requests: hydrated });
  } catch (error) {
    console.error('Mentor requests mentor GET error:', error);
